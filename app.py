    ...
    if request.method == 'GET':
        ...
        if user and check_password_hash(user.password, password):
            ...
            if current_user.role in ['MANAGER', 'ADMIN']:
                ...
                return redirect(redirect_route if redirect_route else url_for(default_student_route_function,
                                                                              student_id=0))
        else:
...
def page_not_found(e):
    flash(f'Sorry! You are trying to access a page that does not exist. '
          f'Please contact support if this problem persists.', 'error')
